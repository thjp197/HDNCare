    ...
    <div className='mx-4 sm:mx-[10%]'>
        ...
        <div>
          {mockAppointments.map((appointment, index) => {
            const stylist =
              stylists.find((s) => s._id === appointment.stylistId || s._id === appointment.styId) ||
              localStylists.find((s) => s._id === appointment.stylistId || s._id === appointment.styId)

            const stylistImage = stylist?.image || assets.profile_pic
            const stylistName = stylist?.name || 'Chuyên viên'
            return (
              <div key={index} className='grid grid-cols-1 md:grid-cols-4 gap-4 border border-gray-200 rounded-lg p-4 mb-4 bg-white shadow-sm hover:shadow-md transition'>
                <div>
                  <img src={stylistImage} alt={stylistName} className='w-32 h-32 object-cover rounded' />
                </div>

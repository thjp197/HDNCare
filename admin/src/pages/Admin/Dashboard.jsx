  ...
  return dashData && (
    <div className='m-5 space-y-6 font-sans'>
      ...
      <div className='rounded-2xl border border-rose-100 bg-gradient-to-r from-rose-50 via-pink-50 to-amber-50 p-5 shadow-sm'>
        <p className='text-sm text-rose-600/80 font-sans'>Bảng điều khiển quản trị</p>
        <h2 className='mt-1 text-2xl font-bold font-sans text-gray-800'>Tổng Quan Hoạt Động HDNCare</h2>
      </div>
        ...
        <div className='group relative overflow-hidden rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg'>
          ...
          <div className='relative flex items-center gap-4'>
            <div className='rounded-xl bg-rose-50 p-2'>
              <img className='w-12 font-sans' src={assets.stylist_icon} alt='Chuyen vien' />
            </div>
            <div>
      ...
      <div className='overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm'>
        <div className='flex items-center gap-2.5 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white px-4 py-4'>
          ...
          <p className='text-base font-semibold text-gray-800'>Lịch Hẹn Mới Nhất</p>
        </div>
        ...
        <div className='divide-y font-sans divide-gray-100'>
          {dashData.latestAppointments.slice(0, 5).map((item, index) => (
            <div className='flex items-center gap-3 px-6 py-3 transition-colors hover:bg-rose-50/40' key={index}>
              <img className='w-10 rounded-full ring-2 ring-white' src={item.styData.image} alt='Anh chuyen vien' />
              <div className='flex-1 text-sm'>
                <p className='font-medium font-sans text-gray-800'>{item.styData.name}</p>
                <p className='text-gray-600 font-sans'>Đặt Lịch: {slotDateFormat(item.slotDate)}</p>
              </div>
              {
                item.cancelled ? (
                  <p className='rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-red-500 font-sans'>Đã Hủy</p>
                ) : item.isCompleted ? (
                  <p className='rounded-full bg-green-50 px-3 py-1 text-xs font-semibold text-green-600 font-sans'>Hoàn Thành</p>
                ) : (
                  <img
